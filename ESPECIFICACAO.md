...
- **Modo escuro**: `@media (prefers-color-scheme: dark)` em `public/css/style.css` ativa tema escuro automaticamente. Cobre cards, inputs, tabelas, ranking, modal e toasts via variáveis CSS (`--bg-page`, `--bg-card`, `--border-soft`, `--text-muted`).
