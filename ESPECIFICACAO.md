...
- **Ordenação de jogos**: A view `/jogos` ordena por `j.data, j.id` (não por `j.id`) para garantir que mata-mata apareçam em ordem cronológica. Jogos finalizados são ordenados do **mais recente para o mais antigo** (`data DESC`) em todas as telas que os exibem: `/jogos`, `/palpites` (sortEntriesPorData), `/resumo` (`j.data DESC`), e `/ranking` ("últimos concluídos", `j.data DESC LIMIT 10`). A única exceção é a view de palpites de um participante (`/ranking/usuario/:id`), que mantém ordem cronológica ascendente por rodada/data por ser uma visão geral de todos os palpites.
