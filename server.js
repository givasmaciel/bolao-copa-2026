...
// Força correção jogo 1 (teste)
app.get('/debug/fix1', async (req, res) => {
  const d = new Date('2026-06-11T19:00:00Z');
  const r = await dbRun("UPDATE jogos SET data = ? WHERE id = ?", [d, 1]);
  const j = await get('SELECT id, data FROM jogos WHERE id = 1');
  res.json({ changes: r.changes, jogo1: j });
});
